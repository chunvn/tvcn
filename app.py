...
st.caption(
    ...
    "add 'Số lần gặp' → keep only required columns/ordering → sort with SĐT groups preserved → merge group cells."
)

uploaded = st.file_uploader("Upload your Excel (.xlsx)", type=["xlsx"])

# Defaults (selectable after upload)
tvcn_sheet_default = "TVCN"
kh_sheet_default   = "Khách hàng"

# EXACT columns to keep (and order)
DESIRED_ORDER = [
...
def keep_only_desired_columns(df):
    """Keep ONLY the DESIRED_ORDER columns (create empty if missing)."""
    out = df.copy()
...
def sort_preserving_sdt_groups(df):
    """
    - Non-empty SĐT rows are grouped; inside each group, sort by 'Ngày thực hiện' ascending.
    - Groups themselves are ordered by 'Ban' ascending (group key = min(Ban) as string).
    - Empty SĐT rows are sorted by Ban then Ngày thực hiện and interleaved by Ban.
    """
    df = df.copy()
    df["_dt_th"] = pd.to_datetime(df["Ngày thực hiện"], errors="coerce")  # set dayfirst=True if needed

    # Split by presence of SĐT
    ...
    groups.sort(key=lambda t: (t[0], t[1]))  # sort groups by Ban, then SĐT

    df_without_sorted = df_without.sort_values(by=["Ban", "_dt_th"], na_position="last", kind="stable")

    # Interleave by Ban key so overall sheet is Ban-ascending, keeping SĐT blocks intact
    items = []
    ...
    items.sort(key=lambda t: (t[0], not t[1]))  # groups first when same Ban
...
if uploaded:
    ...
    if st.button("Run merge"):
        try:
            ...
            # Count, then keep ONLY requested columns (in exact order)
            df_counted = add_count(df_filtered)
            df_kept    = keep_only_desired_columns(df_counted)

            # Sort while preserving SĐT groups + per-group date asc
            df_final = sort_preserving_sdt_groups(df_kept)
            ...
            st.success(
                ...
                "Columns restricted to required list, SĐT treated as text, grouped & sorted, and merged per SĐT."
            )
            st.dataframe(df_final[DESIRED_ORDER].head(100))
            st.download_button(
